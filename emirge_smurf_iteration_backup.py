...
class EmirgeIteration(object):
    ...
    def __init__(self, working_dir,
                 ...
                 # thresholds:
                 min_changed_bases_rate_for_split=0,  # (2.5%)
                 min_coverage_for_split=0.005,  # (0.5%)
                 min_minor_prob_for_split=0.08,
                 min_similar_bases_rate_for_merge=0.98,  # (98%)
        ...
        :param min_changed_bases_rate_for_split: (# of changed bases in the split reference)/( read length)
        ...
        self.th = EmirgeThresholds(min_changed_bases_rate_for_split,
                                   min_coverage_for_split,
                                   min_minor_prob_for_split,
                                   min_similar_bases_rate_for_merge,
                                   max_priors_diff_for_stability_test)
    ...
    def _get_new_references(self, test):
        ...
        logging.info("splitting {} references".format(len(second_best_match)))
        second_best_match[CurrentStateFormat.Reference_id].update(
            second_best_match[CurrentStateFormat.Reference_id].apply(lambda r: str(r) + "#" + str(self.iteration_index)))
    ...
    def _get_new_posterior(self, max_prob_n_full_data, full_posteriors):
        ...
        new_posteriors[CurrentStateFormat.Reference_id].update(new_posteriors[CurrentStateFormat.Reference_id].apply(lambda r: str(r) + "#" + str(self.iteration_index)))
    ...
    @time_it
    def merge_split_and_test_stability(self, probN_dict):
        curr_state_df = pd.DataFrame.from_csv(self.paths.current_state, index_col=None)
        full_posteriors = pd.DataFrame.from_csv(self.paths.posteriors, index_col=None)

        merged_curr_state_df, merged_posteriors = self.merge_references(curr_state_df, full_posteriors)
        ...
        for i in range(0, 2*self.read_len):
            base_ix_columns.append(str(i))
            bases_columns.append('base_' + str(i))
            second_bases_columns.append('2-base_' + str(i))
            second_probs_columns.append('2-' + str(i))
        ...
        for base_ix in base_ix_columns:
            ...
            #take the second best base, if the probability is larger than the needed for split
            ...
            ranked_cols = prob_n_for_ix.columns.to_series()[arank.values[:, ::-1][:, :2]]
            prob_n_full[['base_' + base_ix, '2-base_' + base_ix]] = pd.DataFrame(ranked_cols, index=prob_n_full.index).applymap(lambda b:b[-1])
        ...
        max_prob_n_full_data = prob_n_relevant.merge(merged_curr_state_df, on=[HeadersFormat.Unique_Ref_id, HeadersFormat.Region])
        ...
        max_prob_n_full_data.to_csv("/tmp/max_prob_n_full_data.csv")
        ...
        if self.allow_split and self.iteration_index > 0:
            ...
            max_prob_n_full_data['is_split'] = (max_prob_n_full_data['second_best_counter'] / float(2*self.read_len) >= self.th.min_changed_bases_rate_for_split) \
        ...
        new_refs = self._get_new_references(max_prob_n_full_data)
        new_posteriors = self._get_new_posterior(max_prob_n_full_data, merged_posteriors)


        if self.iteration_index > 0 and self.is_stable_state(new_refs):
            logging.info("Stable state!")
            new_refs['Sequence'] = new_refs.apply(lambda r: bin_to_sequence(r[CurrentStateFormat.Bases.A],
                                                                            r[CurrentStateFormat.Bases.C],
                                                                            r[CurrentStateFormat.Bases.G],
                                                                            r[CurrentStateFormat.Bases.T],
                                                                            2*self.read_len), axis=1)
            new_refs[[CurrentStateFormat.Reference_id,
                      CurrentStateFormat.Region,
                      CurrentStateFormat.Priors,
            ...
            logging.info("{} references found".format(len(new_refs)))
            return False

    @time_it
    def merge_references(self, curr_refs, curr_posteriors):
        curr_refs_len = len(curr_refs)
        # find duplication of regions:
        # example: ref1reg1 -> groupA, ref1reg2 -> groupB, ref2reg1 -> groupC, ref2reg2 -> groupB
        ...
        unique_ref_df_with_priors = unique_ref_df[[CurrentStateFormat.Reference_id,
                                                   CurrentStateFormat.Priors]].drop_duplicates()

        # using the new merged id to calc the merged priors.
        unique_ref_df_with_priors = unique_ref_df_with_priors.groupby(CurrentStateFormat.Reference_id)[CurrentStateFormat.Priors]\
                                                             .sum().reset_index()
        ...
        unique_ref_df = unique_ref_df[CurrentStateFormat.Bases.all + [CurrentStateFormat.Region,
                                                                      ...
                                                                      CurrentStateFormat.Reference_id,
                                                                      ReferenceFormat.Original_Id]]
        unique_ref_df = unique_ref_df.merge(unique_ref_df_with_priors, on=CurrentStateFormat.Reference_id, how='left')
        # keep only one instance of each merged group:
        unique_ref_df.drop_duplicates([CurrentStateFormat.Reference_id, CurrentStateFormat.Region], inplace=True)
        ...
        unique_ref_df = unique_ref_df[CurrentStateFormat.Bases.all + [CurrentStateFormat.Region,
                                                                      ...
                                                                      CurrentStateFormat.Priors,
                                                                      ReferenceFormat.Original_Id]]
        unique_ref_df.rename(columns={ReferenceFormat.Original_Id: CurrentStateFormat.Reference_id }, inplace=True)

        logging.info("Found {}/{} unique reference".format(len(unique_ref_df), curr_refs_len))

        new_posteriors = curr_posteriors.merge(ref_to_unique_groups,
                                               left_on=PosteriorsFormat.Ref_id,
                                               right_on=CurrentStateFormat.Reference_id,
                                               how='left')
        new_posteriors = new_posteriors[[PosteriorsFormat.Posterior, PosteriorsFormat.Ref_id, PosteriorsFormat.Read_id]]
        new_posteriors[PosteriorsFormat.Posterior] = new_posteriors.groupby([PosteriorsFormat.Ref_id, PosteriorsFormat.Read_id])[PosteriorsFormat.Posterior]\
                                                                                                    .transform(sum)
        new_posteriors.drop_duplicates(inplace=True)

        return unique_ref_df.copy(), new_posteriors.copy()
    ...
    def do_iteration(self):
        ...
        self.calc_priors()
        is_stable =  self.merge_split_and_test_stability(prob_n_dict)
        return is_stable
