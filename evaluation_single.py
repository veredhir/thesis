...
def get_expected_df(path):
    ...
    df = df.rename(columns={'sequence': Header.sequence,
                            ...
                            'id': Header.ref_id})
    df[Header.weight] = df.groupby(Header.ref_id)[Header.region].transform('count')
    df[Header.is_changed] = df[Header.ref_id].apply(lambda id: '#' in id)
    df = df[Header.all + [Header.is_changed]]
    validate_priors(df)
...
def main():
    ...
    actual_path = os.path.join(working_dir, ACTUAL_RES_FILE_NAME)
    expected_path = os.path.join(working_dir, EXPECTED_RES_FILE_NAME)

    contain_table, similarity_table, actual_priors, expected_priors = calc_table(expected_path, actual_path)

    calc_precision(similarity_table, actual_priors)
